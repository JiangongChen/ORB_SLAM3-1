...
using namespace std; 
void loadGroundTruth(string str_path, vector<double> &times, vector<g2o::SE3Quat> &gt, int dof); 
void loadTrajectory(string str_path, vector<double> &times, vector<g2o::SE3Quat> &traj, vector<int> &gt_marks, int dof); 
...
int main(int argc, char **argv)
{
    if(argc != 4)
    {
        cerr << endl << "Usage: ./draw_traj path_to_aligned_trajectory path_to_ground_truth DoF" << endl;
        return 1;
    }

    int dof = stoi(argv[3]);
    if (dof!=3 && dof!=6){
        cerr << endl << "only support 3dof and 6dof" << endl;
        return 1;
    }
    ...
    loadTrajectory(argv[1],traj_times,traj,gt_marks,dof); 

    vector<double> gt_times;
    vector<g2o::SE3Quat> gt;
    loadGroundTruth(argv[2],gt_times,gt,dof);
...
void loadGroundTruth(string str_path, vector<double> &times, vector<g2o::SE3Quat> &gt, int dof ){
    ...
    while (!fin.eof())
    {
        ...
        if (dof==6){
            ...
            fin >> ground_truth_time >> tx >> ty >> tz; 
...
void DrawTrajectory(const vector<g2o::SE3Quat> & esti, const vector<g2o::SE3Quat> & gt, vector<int> &gt_marks) {
    ...
    while (pangolin::ShouldQuit() == false) {
        ...
        for (size_t i = 0; i < end_i; i++) {
            ...
            if (gt_marks.size()==0 || gt_marks[i]==0){
                glLineWidth(2);
